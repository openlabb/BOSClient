...
#import "BOSIPv6PrefixResolver.h"
...
- (BOOL)isIPv6OnlyNetwork {
    @synchronized(self) {
        ...
        BOSLogDebug(@"Start resolved network to see if in IPv6-Only env.");
        int localStack = 0;
        ...
        if (localStack & SUPPORT_IPV4_STACK) {
            ...
            // IPv6-Only
            isIPv6Only = YES;
            [[BOSIPv6PrefixResolver getInstance] updateIPv6Prefix];
        } else {
            BOSLogDebug(@"[%s]: Error.", __FUNCTION__);
            isIPv6Only = NO;
        }
        isIPv6OnlyResolved = YES;
        if (isIPv6Only) {
            BOSLogDebug(@"[%s]: IPv6-Only network now.", __FUNCTION__);
        } else {
            BOSLogDebug(@"[%s]: Not IPv6-Only network now.", __FUNCTION__);
        }
        return isIPv6Only;
    }
}
...
- (void)appDidBecomeActiveFunc {
    BOSLogDebug(@"[%s]: App become active, refresh IPv6-Only status.", __FUNCTION__);
...
- (NSString *)handleIpv4Address:(NSString *)addr {
    ...
    if ([self isIPv6OnlyNetwork]) {
        convertedAddr = [[BOSIPv6PrefixResolver getInstance] convertIPv4toIPv6:addr];
    ...
    if (result == 0) {
        ...
        for (int i = 0; i < res->nscount; i++) {
            if (addr_union[i].sin.sin_family == AF_INET) {
                ...
                BOSLogDebug(@"%s: Undefined family.", __FUNCTION__);
            }
        }
        free(addr_union);
    }
