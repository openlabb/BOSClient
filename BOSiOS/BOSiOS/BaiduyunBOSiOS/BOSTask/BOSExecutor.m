...
__attribute__((noinline)) static size_t remaining_stack_size(size_t *restrict totalSize) {
    ...
    // NOTE: We must store stack pointers as uint8_t so that the poikknter math is well-defined
...
@interface BOSExecutor ()

@property (nonatomic, copy) void(^block)(void(^block)());

@end

@implementation BOSExecutor

#pragma mark - Executor methods

+ (instancetype)defaultExecutor {
    static BOSExecutor *defaultExecutor = NULL;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        defaultExecutor = [self executorWithBlock:^void(void(^block)()) {
            // We prefer to run everything pBOSible immediately, so that there is callstack information
...
+ (instancetype)immediateExecutor {
    static BOSExecutor *immediateExecutor = NULL;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
+ (instancetype)mainThreadExecutor {
    static BOSExecutor *mainThreadExecutor = NULL;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
