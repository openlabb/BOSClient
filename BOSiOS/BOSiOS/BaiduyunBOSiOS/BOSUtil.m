...
NSString * const Baiduyun_HOST_SUFFIX = @".bcebos.com";
NSString * const Baiduyun_BOS_TEST_ENDPOINT = @".baiduyun-inc.com";
int32_t const CHUNK_SIZE = 8 * 1024;

@implementation BOSUtil
...
+ (NSData *)constructHttpBodyFromPartInfos:(NSArray *)partInfos {
    ...
    [partInfos enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        if ([obj isKindOfClass:[BOSPartInfo class]]) {
            BOSPartInfo * thePart = obj;
            [body appendFormat:@"<Part>\n<PartNumber>%d</PartNumber>\n<ETag>%@</ETag>\n</Part>\n", thePart.partNum, thePart.eTag];
        }
    }];
    [body appendString:@"</CompleteMultipartUpload>\n"];
    BOSLogVerbose(@"constucted complete multipart upload body:\n%@", body);
...
+ (NSData *)constructHttpBodyForCreateBucketWithLocation:(NSString *)location {
    NSString * body = [NSString stringWithFormat:@"<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
                              ...
                              @"</CreateBucketConfiguration>\n",
                       location];
    BOSLogVerbose(@"constucted create bucket body:\n%@", body);
    return [body dataUsingEncoding:NSUTF8StringEncoding];
}
...
+ (NSString *)getIpByHost:(NSString *)host {
    if ([self isNetworkDelegateState]) {
        BOSLogDebug(@"current network is delegate state");
        return host;
    }
    NSString * ip = [[BOSHttpdns sharedInstance] asynGetIpByHost:host];
    BOSLogDebug(@"resolved host %@ and get ip: %@", host, ip);

    return ip ? [[BOSIPv6Adapter getInstance] handleIpv4Address:ip] : host;
}
...
+ (BOOL)isBOSOriginBucketHost:(NSString *)host {
    return [[host lowercaseString] hasSuffix:Baiduyun_HOST_SUFFIX] || [[host lowercaseString] hasSuffix:Baiduyun_BOS_TEST_ENDPOINT];
}
...
+ (NSString *)sign:(NSString *)content withToken:(BOSFederationToken *)token {
    NSString * sign = [BOSUtil calBase64Sha1WithData:content withSecret:token.tSecretKey];
    return [NSString stringWithFormat:@"BOS %@:%@", token.tAccessKey, sign];
}
...
+ (NSString *)detemineMimeTypeForFilePath:(NSString *)filePath uploadName:(NSString *)uploadName {
    ...
    dispatch_once(&onceToken, ^{
        mimeMap = @{
            ...
            @"x3d": @"application/vnd.hzn-3d-crBOSword",
