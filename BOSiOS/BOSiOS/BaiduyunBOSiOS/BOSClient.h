...
@class BOSGetServiceRequest;
@class BOSCreateBucketRequest;
@class BOSDeleteBucketRequest;
@class BOSHeadObjectRequest;
@class BOSGetBucketRequest;
@class BOSGetBucketACLRequest;
@class BOSGetObjectRequest;
@class BOSPutObjectRequest;
@class BOSPutObjectACLRequest;
@class BOSDeleteObjectRequest;
@class BOSCopyObjectRequest;
@class BOSInitMultipartUploadRequest;
...
@class BOSResumableUploadRequest;
@class BOSTask;
@class BOSExecutor;

@class BOSNetworking;
@class BOSClientConfiguration;
@protocol BOSCredentialProvider;

/**
 BOSClient是BOS服务的iOS客户端，它为调用者提供了一系列的方法，用于和BOS服务进行交互。
 一般来说，全局内只需要保持一个BOSClient，用来调用各种操作。
 */
@interface BOSClient : NSObject
...
/**
 用以收发网络请求
 */
@property (nonatomic, strong) BOSNetworking * networking;
...
/**
 客户端设置
 */
@property (nonatomic, strong) BOSClientConfiguration * clientConfiguration;

/**
 任务队列
 */
@property (nonatomic, strong, readonly) BOSExecutor * BOSOperationExecutor;

/**
 初始化BOSClient，使用默认的本地设置
...
/**
 ...
 用于创建Bucket（不支持匿名访问）。默认情况下，创建的Bucket位于默认的数据中心：BOS-cn-hangzhou。
...
/**
 ...
 用于删除某个Bucket。
 */
- (BOSTask *)deleteBucket:(BOSDeleteBucketRequest *)request;
...
/**
 ...
 Put Object ACL操作通过Put请求中的“x-BOS-object-acl”头来设置，这个操作只有Bucket Owner有权限执行。如果操作成功，则返回200；否则返回相应的错误码和提示信息。
 */
...
/**
 ...
 拷贝一个在BOS上已经存在的object成另外一个object，可以发送一个PUT请求给BOS，并在PUT请求头中添加元素“x-BOS-copy-source”来指定拷贝源。
 BOS会自动判断出这是一个Copy操作，并直接在服务器端执行该操作。如果拷贝成功，则返回新的object信息给用户。
 该操作适用于拷贝小于1GB的文件。
 */
...
/**
 ...
 使用Multipart Upload模式传输数据前，必须先调用该接口来通知BOS初始化一个Multipart Upload事件。该接口会返回一个BOS服务器创建的全局唯一的Upload ID，用于标识本次Multipart Upload事件。
...
/**
 ...
 如果你用同一个part号码，上传了新的数据，那么BOS上已有的这个号码的Part数据将被覆盖。除了最后一块Part以外，其他的part最小为100KB；
 最后一块Part没有大小限制。
 */
...
/**
 ...
 在执行该操作时，用户必须提供所有有效的数据Part的列表（包括part号码和ETAG）；BOS收到用户提交的Part列表后，会逐一验证每个数据Part的有效性。
 当所有的数据Part验证通过后，BOS将把这些数据part组合成一个完整的Object。
 */
